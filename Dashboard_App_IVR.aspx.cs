...
using System.IO;
using System.Net;
public partial class Dashboard_App_IVR : System.Web.UI.Page
{
    ...
    protected void Service_Status()
    {
        ...
        int serviceid = 10000002; // Live 10000001, Stage 10000002
        if (Connection.GetDBMode() == "Live") serviceid = 10000001;
        try
        {
...
,[if].[start]
,[if].[end]
FROM [dbo].[ivr_file] [if] WITH(NOLOCK)
WHERE 1=1
...
FROM [dbo].[ivr_file] [if] WITH(NOLOCK)
WHERE 1=1
AND [if].[start] BETWEEN @sp_date_start AND @sp_date_end
AND [if].[filename] LIKE CASE WHEN @sp_filename IS NULL THEN [if].[filename] ELSE '%' + @sp_filename + '%' END
AND ([if].[invalid] > 0 OR [if].[corrupted] > 0 OR [if].[error] > 0)
)
BEGIN
	SELECT
	TOP (@sp_top)
	[irl].[fileid]
	,(SELECT TOP 1 REPLACE([if].[filename],'.TXT','') FROM [dbo].[ivr_file] [if] WITH(NOLOCK) WHERE [if].[fileid] = [irl].[fileid]) [filename]
	,[irl].[sourceid]
	,[irl].[recordid]
	,[irl].[calldate]
	,[irl].[calltime]
	...
	,[irl].[log]
	,[irl].[logdate]
	FROM [dbo].[ivr_record_log] [irl]
	WHERE 1=1
	AND [irl].[log] NOT IN ('duplicate')
	AND [irl].[fileid] IN (
		SELECT
		[if].[fileid]
		FROM [dbo].[ivr_file] [if] WITH(NOLOCK)
	...
	ORDER BY [irl].[recordid], [irl].[calldate], [irl].[calltime], [irl].[ani]
END
                            ";
                    #endregion Build cmdText
                    cmd.CommandText = cmdText;
    ...
    protected void Data_Record_Details()
    {
        ...
        using (SqlConnection con = new SqlConnection(sqlStr))
        {
            ...
            using (SqlCommand cmd = new SqlCommand("", con))
            {
                ...
                if (tbFileName.Text.Trim().Length > 0)
                {
                    cmd.Parameters.Add("@sp_filename", SqlDbType.VarChar, 20).Value = tbFileName.Text.Trim();
                }
                else
                {
                    cmd.Parameters.Add("@sp_filename", SqlDbType.VarChar, 20).Value = "";
                }
                #endregion SQL Parameters
