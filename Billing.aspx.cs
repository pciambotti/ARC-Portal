...
public partial class Billing : System.Web.UI.Page
{
    ...
    protected void Page_PreInit(Object sender, EventArgs e)
    {
        ...
        {
            String sqlStrARC = Connection.GetConnectionString("ARC_Stage", ""); // ARC_Production || ARC_Stage
            String sqlStrDE = Connection.GetConnectionString("DE_Stage", ""); // DE_Production || DE_Stage
        }
    }
...
FROM (
	...
	WHERE 1=1 AND [dn].[phonenumber] NOT IN (SELECT [dt].[phonenumber] FROM [dbo].[dnis] [dt] WITH(NOLOCK) WHERE [dt].[city] IN ('801','805'))
        ...
        try
        {
            ...
            using (SqlConnection con = new SqlConnection(sqlStrARC))
            {
                ...
                using (SqlCommand cmd = new SqlCommand("", con))
                {
                    ...
                    cmdText += @"
SELECT
ISNULL((SELECT ISNULL([ds].[name],[ds].[displayname]) + ' Transactions' FROM [dbo].[Designation] [ds] WITH(NOLOCK) WHERE [ds].[designationid] = CASE WHEN [d].[designationid] NOT IN (35,158,109,179) THEN [d].[designationid] ELSE NULL END),'Standard Transactions') [type]
,COUNT(DISTINCT([cb].[id])) [count]
,ISNULL(SUM(CASE
	WHEN [cb].[decision] = 'ACCEPT' THEN [cb].[ccauthreply_amount]
	ELSE 0
END),0) [amount_approved]
,ISNULL(CASE WHEN [d].[designationid] NOT IN (35,158,109,179) THEN [d].[designationid] ELSE NULL END,0) [sort]
FROM [dbo].[call] [c] WITH(NOLOCK)
JOIN [dbo].[donationccinfo] [d] WITH(NOLOCK) ON [d].[callid] = [c].[callid]
JOIN [dbo].[cybersource_log_auth] [cb] WITH(NOLOCK) ON [cb].[externalid] = [d].[id]
LEFT OUTER JOIN [dbo].[dnis] [dn] WITH(NOLOCK) ON CASE WHEN LEN([c].[dnis]) = 4 THEN [dn].[dnis] WHEN [dn].[company] = 'DRTV' THEN [dn].[line] ELSE [dn].[phonenumber] END = [c].[dnis]
WHERE 1=1
AND [cb].[createdate] BETWEEN @sp_date_start AND @sp_date_end
AND [cb].[createdate] > '2017-09-07 00:00:00'
AND ([dn].[company] IS NULL OR [dn].[company] NOT IN ('Globetrotters','Special Bio-Med Cross'))
GROUP BY CASE WHEN [d].[designationid] NOT IN (35,158,109,179) THEN [d].[designationid] ELSE NULL END
UNION
SELECT
...
AND [cb].[createdate] BETWEEN @sp_date_start AND @sp_date_end
AND [cb].[createdate] > '2017-09-07 00:00:00'
AND [dn].[company] = 'Special Bio-Med Cross'
UNION
SELECT
